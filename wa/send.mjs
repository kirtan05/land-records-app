...
//   node wa/send.mjs --dry-run                 # show plan, don't connect/send
//   node wa/send.mjs --group="Family"          # match group by name
//   node wa/send.mjs 12036...@g.us             # exact group JID
...
const targets = JSON.parse(readFileSync(join(ROOT, 'survey-input.json'), 'utf8')).filter((r) => r.matched);
...
const { state, saveCreds } = await useMultiFileAuthState(join(ROOT, 'wa/auth'));
