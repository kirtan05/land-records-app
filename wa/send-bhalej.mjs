...
//
//   node wa/send-bhalej.mjs --to="gujarat land" --dry-run
//   node wa/send-bhalej.mjs --to=9198XXXXXXXX          # a phone number = a 1:1 chat
//   node wa/send-bhalej.mjs --to=1203634...@g.us       # an explicit jid
...
const AUTH_DIR = process.env.WA_AUTH_DIR || join(ROOT, 'wa/auth-fam');
...
if (TO.includes('@')) jid = TO;
  ...
  const groups = JSON.parse(readFileSync(join(ROOT, 'wa/groups.json'), 'utf8'));
...
await new Promise((resolve, reject) => {
  ...
  sock.ev.on('connection.update', (u) => {
    if (u.qr) { clearTimeout(t); reject(new Error(`NEEDS_RESCAN — ${AUTH_DIR} is not authenticated; run node wa/login.mjs`)); }
