...
// Everything is written under output/Bhalej_<TOKEN>/; tools/captcha/combine-survey-pdf.py then
// staples integrated + every entry scan into one document.
//
//   node tools/captcha/run-bhalej-full.mjs --surveys=174/p1,174/p3,239
...
const VF6_CACHE = join(ROOT, 'tools/captcha/vf6-cache.json');
...
writeFileSync(join(ROOT, 'tools/captcha/bhalej-run.json'), JSON.stringify({ at: new Date().toISOString(), attempts, runReport }, null, 1));
const acc = attempts.filter((a) => a.accepted).length;
log(`\ncaptcha: ${acc}/${attempts.length} accepted`);
log(`report → tools/captcha/bhalej-run.json`);
py.stdin.end();
await ctx.close();
