...
//   node tools/captcha/fetch-deeds.mjs --surveys=174/p1,174/p2,174/p3,239
...
writeFileSync(join(ROOT, 'tools/captcha/deeds-run.json'), JSON.stringify({ at: new Date().toISOString(), report }, null, 1));
log('\nDEEDS DONE → tools/captcha/deeds-run.json');
py.stdin.end();
await ctx.close();
