...
//
//   node tools/captcha/run-anyror-auto.mjs --n=5              # 5 surveys that have iRCMS cases
//   node tools/captcha/run-anyror-auto.mjs --surveys=23,28    # explicit list
//   node tools/captcha/run-anyror-auto.mjs --n=3 --dry        # cascade + solve, never submit
//
...
const LOG = join(ROOT, 'tools/captcha/anyror-auto-log.json');
...
async function captureEntries(dir) {
  ...
  for (const r of red) {
    ...
    // detail page: no label, zero images (tools/captcha/diag-entry-stale.mjs). Re-firing never helps.
