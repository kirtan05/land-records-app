...
"""Evaluate basic captcha solvers against the tagged samples.
  ...
  tools/captcha/.venv/bin/python tools/captcha/ocr-eval.py [--set ircms|anyror|both]
...
Reports exact-match and per-char accuracy per variant. Ground truth:
  ...
  anyror/labels.csv (human-tagged via tag-anyror.py)
"""
import argparse
import csv
import os
...
# self-contained tessdata (downloaded once into tools/captcha/tessdata)
