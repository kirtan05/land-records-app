...
|---|---|---|---|
| 1 | canonical tokenizer, place ids, uids, content hash | `data/identity/Identity.kt`, `packages/core/identity.mjs` | 206 node checks + 14 JVM tests on one fixture |
| 1–3 | synced schema (16 tables), fixed content-hash column order | `data/sync/SyncSchema.kt`, `packages/core/sync-schema.mjs` | column order asserted table-by-table across both languages |
| 3 | merge engine: LWW + content-hash short-circuit + tiebreak | `data/sync/MergeEngine.kt`, `packages/core/merge.mjs` | 10 shared decision vectors + 10 JVM property tests |
| 3 | storage, export/import bundle, fingerprint | `data/sync/SyncDb.kt`, `packages/core/sync-db.mjs` | 21 end-to-end checks on real SQLite |
| 2 | old-survey ranking + the 5-candidate rule | `data/sync/OldSurveyMatcher.kt`, `packages/core/old-survey-match.mjs` | 6 shared cases + 6 JVM tests |
