...
import { normalizeSurvey, buildCatalog } from '../../core/normalize.mjs';
...
writeFileSync(REPO+'/data/catalog/survey-catalog.json', JSON.stringify({ village: 'Bharoda', count: rows.length, rows }, null, 2));
const csv = 'normalized,display,value\n' + rows.map((r) => `"${r.normalized}","${r.display}","${r.value.replace(/"/g, '""')}"`).join('\n');
writeFileSync(REPO+'/data/catalog/survey-catalog.csv', csv);
