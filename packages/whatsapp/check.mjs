...
sock.ev.on('connection.update', (u) => {
  if (u.qr) finish('NEEDS_SCAN — WhatsApp login expired; run `node packages/whatsapp/login.mjs` to re-scan.', 2);
  if (u.connection === 'open') finish('AUTHED — WhatsApp login is valid, ready to send.', 0);
  if (u.connection === 'close') { const c = u.lastDisconnect?.error?.output?.statusCode; if (c === DisconnectReason.loggedOut) finish('NEEDS_SCAN — logged out; run `node packages/whatsapp/login.mjs`.', 2); }
});
