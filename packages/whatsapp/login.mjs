// Connect to WhatsApp via Baileys: show QR (terminal + packages/whatsapp/qr.png), then list groups.
...
function start() {
  ...
  sock.ev.on('connection.update', async (u) => {
    ...
    if (qr) {
      ...
      try { await QRCode.toFile(BASE + '/qr.png', qr, { width: 520, margin: 2 }); console.log('\n(also saved packages/whatsapp/qr.png — open it if the terminal QR is hard to scan)'); } catch {}
    }
    if (connection === 'open') {
      ...
      console.log('\nGROUPS_SAVED — creds stored in packages/whatsapp/auth (scan once). You can close this.');
      setTimeout(() => process.exit(0), 2000);
    }
    if (connection === 'close') {
      const code = lastDisconnect?.error?.output?.statusCode;
      if (code === DisconnectReason.loggedOut) { console.log('LOGGED_OUT — delete packages/whatsapp/auth and re-run to re-scan.'); process.exit(1); }
