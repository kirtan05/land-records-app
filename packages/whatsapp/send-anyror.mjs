...
//   node packages/whatsapp/send-anyror.mjs --dry-run
//   node packages/whatsapp/send-anyror.mjs --group="gujarat" --slow
...
const order = JSON.parse(readFileSync(join(ROOT, 'data/catalog/survey-input.json'), 'utf8')).filter((r) => r.matched).map((r) => r.normalized);
...
const { state: auth, saveCreds } = await useMultiFileAuthState(join(ROOT, 'packages/whatsapp/auth'));
