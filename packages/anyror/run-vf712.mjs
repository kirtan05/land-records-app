...
//   node packages/anyror/run-vf712.mjs --launch                 # all surveys
//   node packages/anyror/run-vf712.mjs --launch --only=221/p    # just one (validation)
import { chromium } from 'playwright-core';
import { readFileSync, writeFileSync, existsSync, mkdirSync, rmSync } from 'node:fs';
import { join } from 'node:path';
import { execFileSync } from 'node:child_process';
import { surveyToken, normalizeSurvey } from '../core/normalize.mjs';
...
let targets = JSON.parse(readFileSync(REPO+'/data/catalog/survey-input.json', 'utf8')).filter((r) => r.matched);
