# CAPTCHA automation (`packages/captcha/`)
...
- `run-village-ircms.mjs` — full-village Bharoda runner (no human). Resumable via
  ...
  `node packages/captcha/run-village-ircms.mjs [--only=221/p] [--from=400]`
...
- `tesseract` needs `TESSDATA_PREFIX=packages/captcha/tessdata` (set inside `ocr-eval.py`).
