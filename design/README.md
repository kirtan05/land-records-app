...
This replaces the placeholder in `apps/android/app/src/main/java/com/landrecords/app/MainActivity.kt`.
...
Settings. `apps/android/app/src/main/res/values/colors.xml` and `values-night/colors.xml`
...
3. **The live WebView** below. The injected script must:
   - pre-select record type + district/taluka/village/survey (the proven cascade in
     `packages/anyror/*.mjs`) and render those fields as **filled and locked** — reduced
